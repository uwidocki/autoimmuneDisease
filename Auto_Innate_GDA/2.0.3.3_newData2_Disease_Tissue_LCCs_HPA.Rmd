...
tissue_consensus_data = fread("/Users/ursulawidocki/Desktop/BarabasiLab/Data/rna_tissue_consensus.tsv") # GTEx + HPA
...
write.table(data, "/Users/ursulawidocki/Desktop/BarabasiLab/Data/Disease_Tissue_TPM_LCCs_HPA_newData2_Q50.tsv", row.names = F, col.names = T, sep = "\t")
```

```{r}
#data <- fread("/Users/ursulawidocki/Desktop/BarabasiLab/Data/Disease_Tissue_TPM_LCCs_HPA_newData2_Q50.tsv")
data <- fread("/Users/ursulawidocki/Desktop/BarabasiLab/Data/Disease_Tissue_LCCs_HPA_newData2_Q50.tsv")
```

# Organize tissues into subsets
```{r}
brain_tiss <- c("amygdala", "angular gyrus", "angular gyrus (white matter)", "anterior cingulate cortex, supragenual-dorsal
", "anterior cingulate cortex, supragenual-ventral", "anterior cingulate gyrus, pregenual-dorsal", "anterior cingulate gyrus, pregenual-ventral", "anterior cingulate gyrus, subgenual", "anterior cochlear nucleus, ventral", "anterior funiculus (white matter)", "anterior insular cortex, dorsal", "anterior insular cortex, ventral", "anterior thalamic nucleus, ventral")

other_tiss <- c("adipose tissue", "adrenal gland", "appendix")
```
...
temp <- rbind(temp, data.frame(Disease_Name = "narcolepsy", n = 0))
...
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/Disease-Tissue_LCC_TPM_trial_HPA_Q50.pdf", width = 25, height = 20)
...
# subset of above
```{r}
some_diseases <- c("colitis ulcerative", "multiple sclerosis", "lupus erythematosus systemic", "crohn disease", "diabetes mellitus type 1", "psoriasis")

some_tissues <- c("adrenal gland", "bone marrow", "colon", "duodenum", "lymph node", "pancreas", "parathyroid gland", "rectum", "skeletal muscle", "skin", "small intestine", "smooth muscle", "spleen", "stomach", "thymus", "thyroid gland", "spinal trigeminal nucleus, caudal", "spinal trigeminal nucleus, interpolar", "spinal trigeminal nucleus, oral", "spinal vestibular nucleus")

subset_data <- data %>%
  filter(Disease_Name %in% some_diseases) %>%
  filter(Tissue %in% some_tissues)
...
temp <- subset_data %>%
  ...
  dplyr::summarise(n=n())
#temp$n <- as.integer(temp$n)
temp <- rbind(temp, data.frame(Disease_Name = "diabetes mellitus type 1", n = 0))
...
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/Disease-Tissue_LCC_trial_HPA_Q50_subset.pdf", width = 12, height = 8)

((pl3 + plot_spacer() + plot_layout(widths = c(5,1))) / ((pl1 + pl2) + plot_layout(widths = c(5,1))))+ plot_layout(heights = c(1,6))
...
write.table(data, "/Users/ursulawidocki/Desktop/BarabasiLab/Data/Disease_Tissue_LCCs_HPA_newData2_Q25.tsv", row.names = F, col.names = T, sep = "\t")
```
...
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/Disease-Tissue_LCC_trial_HPA_Q25.pdf", width = 25, height = 20)
# patchwork things together
...
write.table(data, "/Users/ursulawidocki/Desktop/BarabasiLab/Data/Disease_Tissue_LCCs_HPA_newData2_Q75.tsv", row.names = F, col.names = T, sep = "\t")
```
#plots
```{r}
p1 <- data %>%
...
temp <- data %>%
  ...
  dplyr::summarise(n=n())
temp <- rbind(temp, data.frame(Disease_Name = "narcolepsy", n = 0))
temp <- rbind(temp, data.frame(Disease_Name = "alopecia areata", n = 0))
temp <- rbind(temp, data.frame(Disease_Name = "crohn disease", n = 0))
temp <- rbind(temp, data.frame(Disease_Name = "diabetes mellitus type 1", n = 0))
temp <- rbind(temp, data.frame(Disease_Name = "multiple sclerosis", n = 0))
temp <- rbind(temp, data.frame(Disease_Name = "lupus erythmatosus systemic", n = 0))
temp <- rbind(temp, data.frame(Disease_Name = "scleroderma", n = 0))
temp <- rbind(temp, data.frame(Disease_Name = "uveitis", n = 0))
temp <- rbind(temp, data.frame(Disease_Name = "arthritis rheomatoid", n = 0))
...
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/Disease-Tissue_LCC_trial_HPA_Q75.pdf", width = 25, height = 20)
# patchwork things together
...
```{r}
# Examine autoimmune disease-tissue comorbidity
...
for(i in 1:(length(tissues)-1)){
  ...
  for(j in (i+1):(length(diseases))){
    ...
    dis_tiss_comorb_df[[k]] <- data.frame(Disease1 = dis1, Disease2 = dis2, NumTissDis1 = length(tiss_dis1), NumTissDis2 = length(tiss_dis2), TissueComorbidity = com_score) # save this as a df, not vect
    
    
  }
}
dis_tiss_comorb_df %<>% bind_rows()
```

# graph the disease-tissue comorbidity above
```{r}
# graph the above

df_for_graph <- dis_tiss_comorb_df %>%
  select(Disease1, Disease2, TissueComorbidity) %>%
  filter(TissueComorbidity != 0) %>%
  rename(weight = TissueComorbidity)

tiss_comb_g <- graph_from_data_frame(df_for_graph, directed = F) %>% simplify()
V(tiss_comb_g)$degree <- degree(tiss_comb_g)


# I want to plot it such that the higher the comorbidity score, the closer the nodes are together
#set.seed(1255)
coord <- tiss_comb_g %>%
  layout_with_drl(., weights = E(.)$weight * 2) %>%
  igraph::norm_coords()

#E(tiss_comb_g)$weight <- E(tiss_comb_g)$TissueComorbidity * 10

CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/Autoimmune_Disease_Tissue_Comorb_HPA_Q50_2.pdf", height = 10, width = 14)
plot(x = tiss_comb_g,
     vertex.frame.color = NA,
     vertex.label.cex = V(tiss_comb_g)$degree / 12,
     ...
     edge.width = E(tiss_comb_g)$weight * 2.5,
     edge.curved = 0.3,
