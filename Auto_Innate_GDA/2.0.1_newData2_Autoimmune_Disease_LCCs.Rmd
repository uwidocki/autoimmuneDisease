...
```{r}
# Read in immunity data
immune_data = read.table("/Users/ursulawidocki/Desktop/BarabasiLab/Data/immunity_genes_all.tsv", sep = "\t", stringsAsFactors = F, header = T, row.names = 1)
...
write.table(autoimmune_LCC_df, file = "/Users/ursulawidocki/Desktop/BarabasiLab/Data/AutoDis_LCCs_new2.tsv", sep = "\t", row.names = F, col.names = T)
```
...
sep_whole_ppi <- NetSci::separation_Significance(ppi_g,
                                    ST = GDA_auto,
                                    Threads = 10,
                                    N = 1000,
                                    correct_by_target = F)

sep_signif_whole_ppi <- sep_whole_ppi %>% 
  mutate(Sab_padj = p.adjust(pvalue_lt, method = "fdr"))
colnames(sep_signif_whole_ppi)[4] <- "Sab_p"

write.table(sep_signif_whole_ppi, "/Users/ursulawidocki/Desktop/BarabasiLab/Data/Autoimmune_Sep_Signif_whole_ppi_new2.tsv", sep = "\t", row.names = T, col.names = T)
```
...
# edit sep
colnames(sep_signif_whole_ppi) <- c("dis1", "dis2", "Sab", "p_Sab","padj_Sab")

# merge them all
jacc_sep_full <- full_join(sep_signif_whole_ppi, jacc_full_df)

write.table(jacc_sep_full, "/Users/ursulawidocki/Desktop/BarabasiLab/Data/Autoimmune_jacc_sep2.tsv", sep = "\t", row.names = F, col.names = T)
