...
# num total pathways ##### REMOVE NAs HEREEEEEEEEEEEE
paths <- unique(c(immune_data$GOPathwayInnate, immune_data$KEGGPathwayInnate, immune_data$InnateDBPathway, immune_data$GOPathwayAdaptive, immune_data$KEGGPathwayAdaptive, immune_data$ReactomePathwayInnate, immune_data$ReactomePathwayAdaptive))
paths <- paths[!is.na(paths)]
length(paths)
# na.omit(datacollected) 

# num innate immune system pathways
inn_paths <- unique(c(immune_data$GOPathwayInnate, immune_data$KEGGPathwayInnate, immune_data$InnateDBPathway, immune_data$ReactomePathIDInnate))
inn_paths <- inn_paths[!is.na(inn_paths)]
length(inn_paths)
# num adaptive immune system pathways
adapt_paths <- unique(c(immune_data$GOPathwayAdaptive, immune_data$KEGGPathwayAdaptive, immune_data$ReactomePathIDAdaptive))
...
# GO innate genes
temp <- immune_data %>% 
  select(Symbol, isInnateGO) %>%
  filter(isInnateGO == 1)
length(unique(temp$Symbol))

# GO adaptive genes
temp <- immune_data %>% 
  select(Symbol, isAdaptiveGO) %>%
...
# KEGG innate genes
temp <- immune_data %>% 
  select(Symbol, isInnateKEGG) %>%
  filter(isInnateKEGG == 1)
length(unique(temp$Symbol))

# KEGG adaptive genes
temp <- immune_data %>% 
  select(Symbol, isAdaptiveKEGG) %>%
...
react_all = immune_data %>%
  filter(ReactomeDB == 1) %>%
  select(Symbol, ReactomeDB) %>%
  unique() %>%
  pull(Symbol)


fit <- euler(list(GO = go_all, KEGG = kegg_all, InnateDB = innatedb_all, ReactomeDB = react_all))
plot(fit,
...
```{r}
# get an example of a gene in innate and adaptive 

immune_data %>%
  filter(isAdaptiveGO == 1) %>%
  filter(isAdaptiveKEGG == 1) %>%
  filter(isInnateGO == 0) %>%
  filter(isInnateKEGG == 0) %>%
  filter(isInnateInnateDB == 0)
...
go_temp = immune_data %>% filter(GO == 1) %>% unique() %>% select(Symbol)
go_temp$Database = "GO"

innatedb_temp = immune_data %>% filter(InnateDB == 1) %>% unique() %>% select(Symbol)
innatedb_temp$Database = "InnateDB"

kegg_temp = immune_data %>% filter(KEGG == 1) %>% unique() %>% select(Symbol)
kegg_temp$Database = "KEGG"

react_temp = immune_data %>% filter(ReactomeDB == 1) %>% unique() %>% select(Symbol)
...
CairoPDF("Database_Jaccard_Imm_Components.pdf", width = 25, height = 15)
plot
dev.off()
...
# Testing for statistical significance between set overlaps

data = matrix(data=c(1,0,0,0,0,7, 5,6,8,8,5,8), nrow=6)

colnames(data) = c("intersection", "universe")
rownames(data) = c("dis_1", "dis_2", "dis_3", "dis_4", "dis_5", "dis_6")
...
col_labels <- c("DB.1", "DB.2", "p_value")
fischer_db_df <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(fischer_db_df) <- col_labels

# Data_db
# for(i in 1:(length(databases)-1)){
  # for(j in (i+1):length(databases)){
  
  # from Data_db, get rows of i and j
  # 
...
for(i in 1:(length(databases)-1)){
  for(j in (i+1):length(databases)){
    ...
    genes_i <- unique(Data_db[which(Data_db$Database == databases[i]),][2])
    genes_j <- unique(Data_db[which(Data_db$Database == databases[j]),][2])
    ...
    #size_collected <- length(unique(Data_db$Symbol))
...
#LCC_go_distrib <- as.data.frame(LCC_go_all$distribution)
#LCC_go_distrib %>%
#  summarize(n=n()) %>%
#  ggplot()+
#  aes()
...
#ggplot(LCC_go_all_df) +
  #aes(x = Degree, y = n) +
  #geom_point(size = 1L, colour = "#bd3786") +
  #scale_x_continuous(trans = "log10") +
  #scale_y_continuous(trans = "log10") +
  #labs(x = "Degree", y = "Freq.", title = "Degrees of GO Proteins in LCC") +
  #theme_minimal()
...
CairoPDF("GO_LCC_plots.pdf", width = 25, height = 20)
...
hist(LCC_innatedb_all$distribution, main = "InnateDB Immune System Empirical LCC Distribution", 
     xlab = "LCC Size", ylab = "LCC Frequency",
     xlim = c(350, 900), ylim = c(0, 150), col = 'gray75', breaks = 20)
...
# ReactomeDB LCC
```{r}
ppi_reactome_all = react_all[react_all %in% V(ppi_g)$name]

LCC_reactome_all = LCC_signif(G = ppi_g, targets = ppi_reactome_all, num_bins_degree_G = 1, min_bin_degree = 20, iter = 1000)

## Plotting
lim = c(LCC_reactome_all$size, LCC_reactome_all$distribution)
hist(LCC_reactome_all$distribution, main = "ReactomeDB Whole Immunity Empirical LCC Distribution", xlim = c(min(lim - 50), max(lim)+50), ylim = c(0, 300), col = 'gray75', ylab = "", breaks = 20)
...
## Degree distribution of genes in the LCC
reactome_all_graph <- induced_subgraph(ppi_g, ppi_reactome_all)
temp <- components(reactome_all_graph)$membership
inLCC_reactome_all = names(temp[temp == 1])

LCC_reactome_all_df <- subset(ppi_degree_df, rownames(ppi_degree_df) %in% inLCC_reactome_all) %>%
  group_by(Degree) %>%
  summarize(n = n())
...
nonLCC_reactome_all_df <- subset(ppi_degree_df, rownames(ppi_degree_df) %in% nonLCC_reactome_all) %>%
  group_by(Degree) %>%
  summarize(n = n())
...
#LCC_test = NetSci::LCC_Significance(N = 1000, G = ppi_g, Targets = ppi_inter, bins = 1)
...
hist(LCC_inter$distribution, main = "Intersection of Immune Databases Empirical LCC Distribution", 
     xlab = "LCC Size", ylab = "LCC Frequency",
     xlim = c(-25, 150), ylim = c(0, 450), col = 'gray75', breaks = 20)
...
nonLCC_inter = names(temp[temp != 2])
