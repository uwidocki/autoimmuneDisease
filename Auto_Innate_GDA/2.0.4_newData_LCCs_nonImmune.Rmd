...
ggplot(non_immune_LCC_df %>% dplyr::arrange(p_val)) +
  ...
  scale_color_manual(values = c("padj < 0.05" = "#004E98", "padj > 0.05" = "#8D99AE")) +
  scale_y_log10() +
  #ylim(0, 7.5) +
  theme_minimal() +
...
```{r}
# Save the LCC output
write.table(non_immune_LCC_df, "/Users/ursulawidocki/Desktop/BarabasiLab/Data/AutoDis_nonImmune_LCCs.tsv", sep = "\t", row.names = T, col.names = T)
...
CairoPDF("./Figures/AutoDis_LCC_Immune.pdf", height = 6, width = 10)
plt
dev.off()
...
```{r}
temp_full_LCC_df <- autoimmune_LCC_df[,c("Disease", "LCCsize","LCC_prop", "p_val", "padj")]
colnames(temp_full_LCC_df) <- c("Disease", "LCCsize_full","LCC_prop_full", "p_val_full", "padj_full")
...
# Check modules in innate immunity

# Check modules in adaptive immunity
