...
# Prevalence and PubMed frequency data
PubMed_freq_data <- fread('/Users/ursulawidocki/Desktop/BarabasiLab/Data/Autoimmune_Disease_Prev.csv', stringsAsFactors=F)
PubMed_freq_data %<>% dplyr::select(Disease, PubMed_papers)
PubMed_freq_data$PubMed_papers <- as.numeric(PubMed_freq_data$PubMed_papers)
...
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/Autoimmune_Prevalence_2.pdf", height = 8, width = 10)
prev_plot
dev.off()
```
...
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/Dis_Prev_vs_NumGenes2.pdf", height = 14, width = 16)
plot_prev
dev.off()
```
...
M.lm=lm(num_genes ~ prev, data = dis_prev_LCC)
summary(M.lm)
```
...
# how does the numGenes relate to PubMed papers
```{r}
dis_PubMed <- left_join(autoimmune_LCC_df, PubMed_freq_data) %>% na.omit()
rownames(dis_PubMed) <- dis_PubMed$Disease

dis_PubMed <- dis_PubMed %>%
  arrange(num_genes)
dis_PubMed$Disease = factor(row.names(dis_PubMed), 
                                   levels = row.names(dis_PubMed))
                                   
ggplot(dis_PubMed) +
  aes(x = PubMed_papers, y = num_genes, colour = signif, size = LCCsize) +
  geom_point(shape = "circle", alpha = 0.5) +
  scale_size(range = c(0, 10)) +
  #scale_y_log10() +
  labs(x = "Num disease genes", y = "Number of PubMed Papers", title = "Statistical Significance of Autoimmune Disease Modules and Number of Disease Genes") +
  scale_color_manual(values = c("padj < 0.05" = "#FB5607", "padj > 0.05" = "#8D99AE")) +
  theme_minimal() + 
  theme(text = element_text(size = 17),
        #axis.text = element_text(size = 15),
...
ggplot(dis_PubMed) +
  aes(x = PubMed_papers, y = num_genes, colour = signif, size = LCCsize, label = Disease) +
  geom_point(shape = "circle", alpha = 0.5) +
  geom_text() +
  scale_size(range = c(0, 10)) +
  #scale_y_log10() +
  labs(x = "PubMed Papers", y = "Number of Disease Genes", title = "Statistical Significance of Autoimmune Disease Modules and Number of Disease Genes") +
  scale_color_manual(values = c("padj < 0.05" = "#FB5607", "padj > 0.05" = "#8D99AE")) +
  theme_minimal() + 
  theme(text = element_text(size = 17),
        #axis.text = element_text(size = 15),
        legend.position = "bottom") +
  geom_abline(slope = 0.001644, intercept = 0.02)


M.lm=lm(num_genes ~ PubMed_papers, data = dis_PubMed)
summary(M.lm)
```
