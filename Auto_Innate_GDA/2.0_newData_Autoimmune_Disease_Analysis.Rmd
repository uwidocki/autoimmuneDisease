...
plot <- gene_assoc %>% filter(n > 6) %>%
  ...
  theme_minimal() +
  coord_flip() 
CairoPDF("./Figures/Num_Genes_per_Disease_filtered6.pdf", width = 6, height = 6)
plot
dev.off()
...
plot <- gene_assoc %>% filter(n > 2) %>%
  ...
  theme_minimal() +
  coord_flip()
CairoPDF("./Figures/Num_Genes_per_Disease_filtered2.pdf", width = 10, height = 25)
plot
dev.off()
...
dim(gene_assoc %>% filter(n == 1))[1] # 1290

# number of genes assoc with multiple diseases
dim(gene_assoc %>% filter(n > 1))[1] # 444

# number of genes with multiautoimmune syndrome potential
dim(gene_assoc %>% filter(n >= 3))[1] # 163

dim(gene_assoc %>% filter(n >= 7))[1] # 36
...
CairoPDF("./Figures/Autoimmune_dis_Jacc.pdf", height = 14, width = 18)
plot
dev.off()
...
CairoPDF("./Figures/Autoimmune_dis_Jacc_filt.pdf", height = 16, width = 25)
plot
dev.off()
...
pdf("./Figures/Autoimmune_dis_Jacc_heatmap.pdf", height = 6, width = 9)
superheat(jacc_dis_mat_full, pretty.order.rows = T, pretty.order.cols=T, scale=F, 
          heat.pal = c("white", "red"), heat.pal.values = c(0, 0.07, 1), 
...
enrich_clust_df <- enrich_inter$result %>% mutate(term_name = tolower(term_name))
...
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/potential_MAS.pdf", height = 16, width = 25)
plot
dev.off()
```
...
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/cluster_MAS.pdf", height = 16, width = 25)
g_poten_MAS %>% 
  delete.edges(which(E(g_poten_MAS)$weight <= 0.05)) %>%
...
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/MAS_graphs.pdf", 
         height = 60, width = 60)
#patchwork <- 
  (g1 / g2 / g3)
...
```{r}
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/MAS_graphs_horiz.pdf", height = 30, width = 130)
#patchwork <- 
(g1+g2 +g3)
#patchwork + plot_annotation(tag_levels = 'A')
dev.off()
...
dis_immun_bar_df <- dis_in_immunity %>%
  ...
  dplyr::select(NewName, Innate, NumAdapt) %>%
  unique()
dis_immun_bar_df <- as.data.frame(dis_immun_bar_df)
rownames(dis_immun_bar_df) <- dis_immun_bar_df$NewName
dis_immun_bar_df$NumInnate <- dis_immun_bar_df$Innate + dis_immun_bar_df$NumAdapt

dis_immun_bar_df <- left_join(dis_immun_bar_df, disease_gene_assoc)
colnames(dis_immun_bar_df)[5] <- "num_genes"
dis_immun_bar_df$propI <- dis_immun_bar_df$Innate / dis_immun_bar_df$num_genes
dis_immun_bar_df$propAdapt <- dis_immun_bar_df$NumAdapt / dis_immun_bar_df$num_genes
dis_immun_bar_df$propInnate <- dis_immun_bar_df$propI + dis_immun_bar_df$propAdapt

dis_immun_raw_count_df <- dis_immun_bar_df[,c("NewName", "NumInnate", "NumAdapt", "num_genes")]
dis_immun_prop_df <- dis_immun_bar_df[,c("NewName", "propInnate", "propAdapt", "num_genes")]
...
p1 <- ggplot(dis_immun_raw_count_df) + 
  aes(x = reorder(num_genes), y = numGenes, fill = ImmMod) + 
...
p2 <- ggplot(dis_immun_prop_df) + 
  aes(x = reorder(num_genes), y = propGenes, fill = ImmMod) + 
  geom_bar(stat = "identity", position = "identity") +
  labs(x = "Disease", y = "Fraction of Genes in Innate and Adaptive Immunity", 
       title = "Percentage of Autoimmune Disease Associated Genes in Innate and Adaptive Immunity") +
  coord_flip() +
  theme_minimal() + 
  theme(axis.text = element_text(size = 18),
        title = element_text(size = 18),
...
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/Num_Dis_Genes_per_Immune_RAW.pdf", height = 25, width = 17)
p1 
dev.off()

CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/Num_Dis_Genes_per_Immune_prop.pdf", height = 25, width = 17)
p2
dev.off()
...
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/Num_Dis_Genes_per_Immune_Comp.pdf", height = 25, width = 17)
p1 
dev.off()
...
CairoPDF("/Users/ursulawidocki/Desktop/BarabasiLab/Auto_Innate_GDA/Figures/Num_Dis_Genes_per_Immune_Comp_top10.pdf", height = 20, width = 30)
p1 
dev.off()
```
