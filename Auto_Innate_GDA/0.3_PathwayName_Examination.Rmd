...
immune_data = immune_data %>%
  mutate(GOpathName.innate = tolower(GOpathName.innate)) %>%
  mutate(GOpathName.adaptive = tolower(GOpathName.adaptive)) %>%
  mutate(InnateDBpathName = tolower(InnateDBpathName)) %>%
  mutate(KEGGpathName.innate = tolower(KEGGpathName.innate)) %>%
  mutate(KEGGpathName.adaptive = tolower(KEGGpathName.adaptive)) %>%
  mutate(ReactomePathName.innate = tolower(ReactomePathName.innate)) %>%
  mutate(ReactomePathName.adaptive = tolower(ReactomePathName.adaptive))
...
test_df$GOpathName.innate <- sub("\\s*\\([^\\)]+\\)", "", as.character(test_df$GOpathName.innate))
test_df$GOpathName.innate <- sub("-", " ", as.character(test_df$GOpathName.innate)) # these two lines are not equal
test_df$GOpathName.innate <- sub("-", " ", as.character(test_df$GOpathName.innate)) # from keyboard
test_df$GOpathName.innate <- sub(",", "", as.character(test_df$GOpathName.innate))
test_df$GOpathName.adaptive <- sub("\\s*\\([^\\)]+\\)","", as.character(test_df$GOpathName.adaptive))
test_df$GOpathName.adaptive <- sub("-", " ", as.character(test_df$GOpathName.adaptive))
test_df$GOpathName.adaptive <- sub("-", " ", as.character(test_df$GOpathName.adaptive)) # from keyboard
test_df$GOpathName.adaptive <- sub(",", "", as.character(test_df$GOpathName.adaptive))

test_df$InnateDBpathName <- sub("\\s*\\([^\\)]+\\)", "", as.character(test_df$InnateDBpathName))
test_df$InnateDBpathName <- sub("-", " ", as.character(test_df$InnateDBpathName))
test_df$InnateDBpathName <- sub("-", " ", as.character(test_df$InnateDBpathName)) # from keyboard
test_df$InnateDBpathName <- sub(",", "", as.character(test_df$InnateDBpathName))

test_df$KEGGpathName.innate <- sub("\\s*\\([^\\)]+\\)", "", as.character(test_df$KEGGpathName.innate))
test_df$KEGGpathName.innate <- sub("-", " ", as.character(test_df$KEGGpathName.innate))
test_df$KEGGpathName.innate <- sub("-", " ", as.character(test_df$KEGGpathName.innate)) # from keyboard
test_df$KEGGpathName.innate <- sub(",", "", as.character(test_df$KEGGpathName.innate))
test_df$KEGGpathName.adaptive <- sub("\\s*\\([^\\)]+\\)", "", as.character(test_df$KEGGpathName.adaptive))
test_df$KEGGpathName.adaptive <- sub("-", " ", as.character(test_df$KEGGpathName.adaptive))
test_df$KEGGpathName.adaptive <- sub("-", " ", as.character(test_df$KEGGpathName.adaptive)) # from keyboard
test_df$KEGGpathName.adaptive <- sub(",", "", as.character(test_df$KEGGpathName.adaptive))

test_df$ReactomePathName.innate <- sub("\\s*\\([^\\)]+\\)","", as.character(test_df$ReactomePathName.innate))
test_df$ReactomePathName.innate <- sub("-", " ", as.character(test_df$ReactomePathName.innate))
test_df$ReactomePathName.innate <- sub("-", " ", as.character(test_df$ReactomePathName.innate)) # from keyboard
test_df$ReactomePathName.innate <- sub(",", "", as.character(test_df$ReactomePathName.innate))
test_df$ReactomePathName.adaptive <- sub("\\s*\\([^\\)]+\\)","", as.character(test_df$ReactomePathName.adaptive))
test_df$ReactomePathName.adaptive <- sub("-", " ", as.character(test_df$ReactomePathName.adaptive))
test_df$ReactomePathName.adaptive <- sub("-", " ", as.character(test_df$ReactomePathName.adaptive)) # from keyboard
test_df$ReactomePathName.adaptive <- sub(",", "", as.character(test_df$ReactomePathName.adaptive))
```
...
```{r}
test_df$KEGGpathName.innate <- sub(pattern = "   homo sapiens", replacement = "", as.character(test_df$KEGGpathName.innate), fixed = T)

test_df$KEGGpathName.adaptive <- sub(pattern = "   homo sapiens", replacement = "", as.character(test_df$KEGGpathName.adaptive), fixed = T)

```

Edit ReactomeDB Path names
```{r}
test_df$ReactomePathName.innate <- sub(pattern = "homo sapiens: ", replacement =  "", as.character(test_df$ReactomePathName.innate), fixed = T)

test_df$ReactomePathName.adaptive <- sub(pattern = "homo sapiens: ", replacement =  "", as.character(test_df$ReactomePathName.adaptive), fixed = T)
...
InnateDBPathwayNames = unique(sub_test_df$newPathName)
InnateDBPathwayNames = InnateDBPathwayNames[!is.na(InnateDBPathwayNames)]

GOPathwayNames = unique(c(test_df$GOpathName.innate, test_df$GOpathName.adaptive))
GOPathwayNames = GOPathwayNames[!is.na(GOPathwayNames)]

KEGGPathwayNames = unique(c(test_df$KEGGpathName.innate, test_df$KEGGpathName.adaptive))
KEGGPathwayNames = KEGGPathwayNames[!is.na(KEGGPathwayNames)]

ReactomePathwayNames = unique(c(test_df$ReactomePathName.innate, test_df$ReactomePathName.adaptive))
ReactomePathwayNames = ReactomePathwayNames[!is.na(ReactomePathwayNames)]

#PathwayNames = unique(c(InnateDBPathwayNames, GOPathwayNames, KEGGPathwayNames, ReactomePathwayNames))
...
fviz_nbclust(see, kmeans, method = "silhouette", k.max = 25) # 7
fviz_nbclust(see, kmeans, method = "wss", k.max = 25) # 6
fviz_nbclust(see, kmeans, method = "gap_stat", k.max = 25) # 6
```

```{r}
km.res <- kmeans(x = see, centers = 6, nstart = 50, iter.max = 20)
...
#km.res <- kmeans(x = see, centers = 7, nstart = 50, iter.max = 20)
