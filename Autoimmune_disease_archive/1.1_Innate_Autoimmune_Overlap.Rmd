---
title: "1.0.1_Innate_Autoimmune_Overlap"
...
innates <- as.character(all_innate_genes_df$Symbol) %>% unique()

fit <- euler(list(autoimmuneDiseases = autos, innateImmunity = innates))
...
fit <- euler(list(autoimmuneDiseases = ppi_auto, innateImmunity = ppi_innate))
...
# Which autoimmune diseases have a significant LCC
```{r}
# Go through each disease and store LCC size and p-value

GDA_disease = unique(GDA_auto$NewName)
GDA_LCC_df = as.data.frame(matrix(0, ncol = 3, nrow = length(GDA_disease)))
colnames(GDA_LCC_df) = c("LCCsize", "LCCp", "numDisGenes")
...
for(dis in GDA_disease){
  ...
  GDA_LCC_df[dis, "numDisGenes"] <- length(unique(genes))
...
```{r}
# Which p-values are duplicated

dup_p_df = GDA_LCC_df[duplicated(GDA_LCC_df$LCCp),]
dup_p = unique(dup_p_df$LCCp)
dup_p_df = GDA_LCC_df[GDA_LCC_df$LCCp %in% dup_p,]

```


```{r}
# make the dot plot/ manhattan-like plot suggested

GDA_LCC_df %<>%
  arrange(LCCp) 

GDA_LCC_df$disease = factor(row.names(GDA_LCC_df), levels = row.names(GDA_LCC_df))

GDA_LCC_df %>% 
  mutate(padj = p.adjust(LCCp, method = "fdr")) %>%
  mutate(signif = ifelse(padj < 0.05, "padj < 0.05", "padj > 0.05")) %>%
  ggplot() +
  aes(x = disease, y = -log10(LCCp), colour = signif, size = LCCsize) +
...
ggplot(GDA_LCC_df %>% dplyr::arrange(LCCp)) +
  aes(x = disease, y = LCCp, colour = signif, size = LCCsize) +
...
ggplot(GDA_LCC_df %>% dplyr::arrange(LCCp)) +
  aes(x = disease, y = -log10(LCCp), colour = signif, size = LCCsize) +
  geom_point(shape = "circle", fill = "#112446", alpha = 0.4) +
  labs(x = "Disease", y = "LCC -log10 p-value") +
  scale_color_hue(direction = 1) +
  scale_y_log10() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  geom_hline(yintercept = -log10(0.05), color = "red")
...
```
In the above plot, we see that there are quite a few diseases have the same p-value. Why is that?

```{r}
# Which diseases have a p-value from 0.499 to 0.510
trend = GDA_LCC_df[GDA_LCC_df$LCCp == GDA_LCC_df["optic neuritis", "LCCp"],] # 9 diseases

# how many genes do they have?
trend_gene_count = subset(GDA_auto, NewName %in% trend$disease) # 9
trend_gene_count$disease = trend_gene_count$NewName
trend_gene_count = trend_gene_count[,c("disease", "Symbol")] %>% unique()
trend_gene_count = trend_gene_count %>%
  group_by(disease) %>%
  summarise(n = n()) 
rownames(trend_gene_count) = trend_gene_count$disease
trend = merge(trend, trend_gene_count, by.y = "disease")

trend %<>%
  arrange(n) 
trend$disease = factor(trend_gene_count$disease, levels = trend_gene_count$disease)

ggplot(trend %>% dplyr::arrange(n)) +
  aes(x = disease, y = n, size = LCCsize) +
   ggplot2::scale_y_continuous(limits = c(0, 4)) + 
  #scale_size(range = c(0, 1)) +
  geom_point(color = "blue", alpha = 0.4) +
  labs(x = "Disease", y = "Number of Disease Genes") +
  theme_minimal() + 
...
```{r}
# What do their gene degree distributions look like?
# What do their LCC emp distributions look like?

for(dis in trend$disease){
  
  genes = GDA_auto[GDA_auto$NewName %in% dis,"Symbol"] %>% unique()
  genes = genes[genes %in% V(ppi_g)$name]
  LCC_trend = LCC_signif(G = ppi_g, targets = genes, num_bins_degree_G = 50, min_bin_degree = 20, iter = 1000)
  
  # LCC plot
  lim = c(LCC_trend$size, LCC_trend$distribution)
  hist(LCC_trend$distribution, main = paste0(dis, " LCC distribution"), 
       xlim = c(min(lim - 50), max(lim + 50)), ylim = c(0, max(LCC_trend$distribution))+10, col = 'gray75', ylab = "", breaks = 20)
  abline(v = LCC_trend$size, col = "red")
  
  #frame()
  #new.plot()
  # gene degree distributions
  genes_df <- subset(ppi_degree_df, rownames(ppi_degree_df) %in% genes)
  plot_genes_df <- genes_df %>%
    group_by(Degree) %>% 
    summarise(n = n())
  
  plot(plot_genes_df$Degree, plot_genes_df$n, main = paste0("Distrib. of Genes in ", dis))
  
  #ggplot(plot_genes_df) +
  #aes(x = Degree, y = n) +
  #geom_point(size = 1L, colour = "#1f9e89") +
  #scale_x_continuous(trans = "log10") +
  #scale_y_continuous(trans = "log10") +
  #labs(x = "Degree", y = "Freq.", title = paste0("PPI Degrees of Genes in ", dis)) +
  #theme_minimal()
...
```{r}
genes_trend = GDA_auto[GDA_auto$NewName %in% trend$disease,]

ggplot(genes_trend) +
  aes(x = Symbol, y = NewName) +
  geom_tile(size = 1.2, fill = "#0C4C8A") +
  labs(x = "Gene Symbol", y = "Disease") +
  theme_light() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
auto_graph <- induced_subgraph(ppi_g, genes_signif_dis)
...
components(auto_graph)$csize

which(components(auto_graph)$csize == 2)
...
```{r}
fit <- euler(list(autoimmuneDiseaseLCC = inLCC_auto, innateImmunityLCC = inLCC_innate))
...
genes_intersect = inLCC_auto[inLCC_auto %in% inLCC_innate] # 445
...
genes_all_innate <- all_innate_genes_df$Symbol %>% unique() 
...
# GO
go_df <- subset(all_innate_genes_df, all_innate_genes_df$GO == "GO.DB")
...
# InnateDB
innate_df <- subset(all_innate_genes_df, all_innate_genes_df$InnateDB == "InnateDB")
...
# KEGG
kegg_df <- subset(all_innate_genes_df, all_innate_genes_df$KEGG == "KEGG.db")
...
# Reactome
reactome_df <- subset(all_innate_genes_df, all_innate_genes_df$ReactomeDB == "ReactomeDB")
...
```{r}
# represent separation as a network
...
```{r}
# Compare with using the embedding

edit_GDA = gsub(pattern = "-", replacement = " ", x = GDA_data$NewName, ignore.case = T)
edit_GDA = gsub("[^0-9A-Za-z ]","" , edit_GDA, ignore.case = TRUE)
bank_GDA = unique(edit_GDA)

doesItWork_auto = gsub(pattern = "-", replacement = " ", x = autoimmune_diseases$AutoDiseases , ignore.case = T)
doesItWork_auto = gsub("[^0-9A-Za-z ]","" , doesItWork_auto,ignore.case = TRUE)
bank_aarda = unique(doesItWork_auto)
...
# make as one list
all_diseases = c(bank_GDA, bank_aarda) %>% unique()

# make into file
write.table(all_diseases,"/Users/ursulawidocki/Desktop/BarabasiLab/Data/all_disease_list.csv",sep=",")
```
...
DiseaseNames_w2v = read.table("/Users/ursulawidocki/Desktop/BarabasiLab/Data/all_disease_list.csv" , sep = ",", header = T, stringsAsFactors = F)
colnames(DiseaseNames_w2v) <- "DiseaseName"
model <- read.word2vec( "/Users/ursulawidocki/Desktop/BarabasiLab/Data/pubmed2018_w2v_400D/pubmed2018_w2v_400D.bin" )
terms <- summary(model, "vocabulary")
embedding <- as.matrix(model) #%>% as.data.frame()
...
c = U %>%t%>% na.exclude() %>%  philentropy::distance(., method = "qgram") # matrix with distance between diseases
xxx = U %>%t%>% na.exclude() %>% row.names()
colnames(c) = row.names(c) = xxx
# 
data.table::fwrite(c, "/Users/ursulawidocki/Desktop/BarabasiLab/Data/disease_name_dist.txt")
